...
The draft poster is located at `poster/poster (1).html`, with a text outline in `poster/outline.md`. It includes the refined research question, motivation, methodology, model variants, planned evaluation metrics, placeholder result visualizations, and related work. The poster layout is essentially complete; the remaining work for the final submission is to replace placeholders with the final larger-scale experimental results.
